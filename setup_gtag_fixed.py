...
# Patrones para limpiar el GTM anterior
...
for file in html_files:
    ...
    # 1. Eliminar GTM (head y body)
    content = gtm_head_pattern.sub('', content)
    content = gtm_body_pattern.sub('', content)
    
    # 2. Insertar gtag.js en el head si no está
    if 'G-68682PBMTQ' not in content:
        content = content.replace('<head>', '<head>\n    ' + gtag_script)

    with open(file, 'w', encoding='utf-8') as f:
        f.write(content)

print("Google Analytics (gtag.js) instalado correctamente.")
